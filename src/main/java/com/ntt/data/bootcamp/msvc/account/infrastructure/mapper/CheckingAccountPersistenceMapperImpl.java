...
public class CheckingAccountPersistenceMapperImpl implements ICheckingAccountPersistenceMapper {
  ...
  public CheckingAccountCollection toEntity(CheckingAccount checkingAccount) {
    BalanceCollection balanceCollection =
        new BalanceCollection(
            ...
            checkingAccount.getBalance().getCurrency().getCurrencyCode(),
            checkingAccount.getBalance().getTimestamp());

    List<AccountHolderCollection> accountHolderCollections =
        checkingAccount.getHolders().stream()
            .map(
                accountHolder ->
    ...
    List<AuthorizedSignerCollection> authorizedSignerCollections =
        checkingAccount.getSigners().stream()
            .map(
                authorizedSigner ->
                    new AuthorizedSignerCollection(
                        authorizedSigner.getDocumentType(), authorizedSigner.getDocumentNumber()))
            .collect(Collectors.toList());
    ...
    return new CheckingAccountCollection(
        checkingAccount.getIdValue(),
        checkingAccount.getCustomerId(),
        checkingAccount.getCustomerType(),
        checkingAccount.getAccountType().name(),
        checkingAccount.getAccountNumber(),
        checkingAccount.getExternalAccountNumber(),
        checkingAccount.getStatus().name(),
        accountHolderCollections,
        balanceCollection,
        checkingAccount.getCreatedAt(),
        checkingAccount.getUpdatedAt(),
        checkingAccount.getMaintenanceFee(),
        checkingAccount.getNextFeeDate(),
        authorizedSignerCollections,
  ...
  public CheckingAccount toDomain(CheckingAccountCollection checkingAccountCollection) {
    AccountHolderCollection accountHolderCollection =
        checkingAccountCollection.getHolders().stream()
    ...
    checkingAccountCollection
        .getHolders()
        .forEach(accountHolderCollect -> log.warn("accountHolderCollect: " + accountHolderCollect));
    ...
    Set<AuthorizedSigner> authorizedSigners =
        checkingAccountCollection.getAuthorizedSigners().stream()
            .map(
                authorizedSignerCollect ->
                    AuthorizedSigner.of(
                        authorizedSignerCollect.getDocumentType(),
                        authorizedSignerCollect.getDocumentNumber()))
            .collect(Collectors.toSet());
    ...
    return CheckingAccount.reconstruct(
        ...
        balance,
        Audit.reconstruct(
            checkingAccountCollection.getCreatedAt(), checkingAccountCollection.getUpdatedAt()),
        accountHolders,
        checkingAccountCollection.getMaintenanceFee(),
        checkingAccountCollection.getNextFeeDate(),
        authorizedSigners,
        transactionLimit);
  }
}
