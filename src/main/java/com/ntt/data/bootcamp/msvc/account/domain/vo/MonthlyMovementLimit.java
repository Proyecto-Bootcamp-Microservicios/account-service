...
public final class MonthlyMovementLimit {
  ...
  private static final int MAX_MONTHLY_LIMIT = 100;
  private static final int UNLIMITED = Integer.MAX_VALUE;

  private MonthlyMovementLimit(int limit, LocalDate monthStartDate, int currentMovements) {
    if (limit < 0) throw new IllegalArgumentException("Limit cannot be negative");
    if (limit != UNLIMITED && limit > MAX_MONTHLY_LIMIT)
      throw new IllegalArgumentException(
          "The limit cannot be exceeded " + MAX_MONTHLY_LIMIT + " movements");
    if (monthStartDate == null)
      throw new IllegalArgumentException("Month start date cannot be null");
    if (currentMovements < 0)
      throw new IllegalArgumentException("Current movements cannot be negative");
    if (currentMovements > limit)
      throw new IllegalArgumentException("Current movements cannot be greater than limit");
    this.limit = limit;
  ...
  public static MonthlyMovementLimit reconstruct(
      int limit, LocalDate monthStartDate, int currentMovements) {
  ...
  public MonthlyMovementLimit syncWithRealCount(int realMovementCount) {
    ...
    if (realMovementCount > this.limit) {
      return new MonthlyMovementLimit(this.limit, currentMonth, 0);
    }
    ...
    return new MonthlyMovementLimit(this.limit, this.monthStartDate, realMovementCount);
  }
