...
public class FixedTermAccountUpdater {
  ...
  public void updateFixedTermAccounts() {
    accountRepositoryPort
        .findByAccountType(AccountType.FIXED_TERM.name())
        .flatMap(
            account -> {
              Account updatedAccount = account.updateOperationDateIfNeeded();
              if (updatedAccount != account) {
                log.info(
                    "Actualizando cuenta {} con nueva fecha de operación",
                    updatedAccount.getAccountNumber());
                return accountRepositoryPort.save(updatedAccount);
              }
              return Mono.just(account);
            })
        .doOnComplete(() -> log.info("Actualización de fechas de operación completada"))
        .doOnError(
            error -> log.error("Error actualizando fechas de operación: {}", error.getMessage()))
        .subscribe();
  }
}
